...
Этот проект объединяет несколько проектов по развитию сообщества u7-hub.
...
Проект использует методологию **Conductor** для управления задачами. Доступны следующие skills (через `/skill:conductor-<command>`):

| Skill | Команда | Описание |
|-------|---------|----------|
| `conductor-setup` | `/skill:conductor-setup` | Первоначальная настройка проекта и Conductor окружения |
| `conductor-newTrack` | `/skill:conductor-newTrack` | Создание нового трека (фичи/багфикса) |
| `conductor-implement` | `/skill:conductor-implement` | Реализация задач из плана трека |
| `conductor-status` | `/skill:conductor-status` | Отображение прогресса проекта |
| `conductor-review` | `/skill:conductor-review` | Ревью реализованного трека |
| `conductor-revert` | `/skill:conductor-revert` | Откат изменений трека |
